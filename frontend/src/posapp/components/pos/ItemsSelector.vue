...
export default {
	...
	data: () => ({
		...
		background_sync_interval: 10,

		temp_background_sync_interval: 10,
	...
	watch: {
		...
		pos_profile: {
			handler(newProfile, oldProfile) {
				// Initialize store when POS profile becomes available
				if (newProfile && newProfile.name && !oldProfile) {
					this.$nextTick(async () => {
						try {
							await this.initializeStore(newProfile, this.customer, this.customer_price_list);
							console.log("Pinia store initialized successfully from POS profile watcher");
						} catch (error) {
							console.error("Failed to initialize store from POS profile watcher:", error);
						}
					});
				}
			},
	...
	methods: {
		...
		async forceReloadItems() {
			...
			try {
				...
				// Force refresh item details to get fresh quantities from server

				// This clears the item details cache and fetches real quantities
				...
				// Rebuild barcode index with fresh items (full list for complete barcode coverage)
		...
		async add_item(item, options = {}) {
			...
			} else if (!reservedStockData.available_qty && !reservedStockData.reserved_qty && item.actual_qty !== undefined && item.actual_qty !== null) {

				// Fallback to item.actual_qty if reserved stock data is empty
		...
		async processScannedItem(scannedCode) {
			...
			// Log why item wasn't found locally

			console.log('Barcode not found locally', {
			...
			try {
				...
				if (qtyFromBarcode !== null) {
					...
					// Regular barcodes use the full items API
		...
		async performBackgroundSync({ source = "manual" } = {}) {
			...
			try {
				...
				// Rebuild barcode index with updated quantities for accurate scanning
		...
		setupWindowFocusSync() {
			...
			// Real-time sync when user switches back to POS tab

			this.handleWindowFocus = () => {

				if (!this.enable_background_sync) return;

				

				// Immediate sync for displayed items (fast)

				if (this.displayedItems && this.displayedItems.length > 0) {

					this.update_items_details(this.displayedItems, { forceRefresh: true });

				}

				// Rebuild barcode index with fresh data (full items list for complete barcode coverage)

				this.replaceBarcodeIndex(this.items);

				// Full background sync (comprehensive)

				this.performBackgroundSync({ source: "window-focus" });
			...
			// Also listen for document visibility changes (more reliable)

			this.handleVisibilityChange = () => {

				if (!document.hidden && this.enable_background_sync) {
	...
	async created() {
		...
		// Setup real-time sync when window gains focus (detect changes from other tabs)
