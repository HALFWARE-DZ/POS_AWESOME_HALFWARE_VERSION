...
<template>
	...
	<!-- Reservation Dialog -->
		<v-dialog v-model="reservation_dialog" max-width="400px">
			<v-card>
				<v-card-title>
					<span class="text-h5 text-primary">{{ __("Reservé - Set Reservation Details") }}</span>
				</v-card-title>
				<v-card-text class="pa-0">
					<v-container>
						<v-text-field
							...
							type="number"
							min="0"
							:max="invoice_doc ? invoice_doc.grand_total : 0"
							class="sleek-field pos-themed-input"
							v-model.number="reservation_amount"
							:label="frappe._('Partial Payment Amount')"
							hide-details
							:prefix="currencySymbol(invoice_doc ? invoice_doc.currency : '')"
						></v-text-field>
						<VueDatePicker
							...
							auto-apply
							class="sleek-field pos-themed-input mt-3"
							:placeholder="frappe._('Reservation Time')"
						/>
						<v-text-field
							class="mt-3 sleek-field"
							density="compact"
							variant="solo"
							type="number"
							min="0"
				...
				<v-card-actions>
					<v-spacer></v-spacer>
					<v-btn color="error" theme="dark" @click="reservation_dialog = false">
						{{ __("Cancel") }}
					</v-btn>
					<v-btn color="primary" theme="dark" @click="confirm_reservation">
...
export default {
	...
	data() {
		return {
			...
			reservation_dialog: false, // Show reservation dialog
			custom_days_value: null, // Custom days entry
			reservation_time: null, // Selected reservation time
	...
	watch: {
		...
		// Watch is_credit_sale to handle reservation toggle
		is_credit_sale(newVal) {
			...
			if (newVal) {
				...
				this.invoice_doc.custom_is_reserve = 1;
				// Set cash payment to 0 for reservation (will be set in dialog)
				...
				this.open_reservation_dialog();
			} else {
				// Disable reservation mode
				this.invoice_doc.custom_is_reserve = 0;
				// Set cash payment to invoice total
				this.invoice_doc.payments.forEach((payment) => {
					if (payment.mode_of_payment.toLowerCase() === "cash") {
						payment.amount = this.invoice_doc.grand_total;
					}
				});
			}
		},
	...
	methods: {
		...
		// Open reservation dialog and initialize values
		open_reservation_dialog() {
			if (!this.invoice_doc) return;
			
			// Set default reservation amount to 0 (user will specify)
			this.reservation_amount = 0;
			// Set default reservation time to current date + 1 day
			const tomorrow = new Date();
			tomorrow.setDate(tomorrow.getDate() + 1);
			this.reservation_time = tomorrow.toISOString().slice(0, 16);
			// Set default due days to 7 days
			this.credit_due_days = 7;
			
			this.reservation_dialog = true;
		},
		// Confirm reservation and set payment amounts
		confirm_reservation() {
			if (!this.invoice_doc) return;
			
			// Validate reservation amount
			if (this.reservation_amount < 0 || this.reservation_amount > this.invoice_doc.grand_total) {
				this.eventBus.emit("show_message", {
					title: __("Invalid reservation amount"),
					color: "error",
				});
				return;
			}
			...
			if (!this.reservation_time) {
				this.eventBus.emit("show_message", {
					title: __("Please select reservation time"),
					color: "error",
				});
				return;
			}
			
			// Set reservation details on invoice
			this.invoice_doc.custom_is_reserve = 1;
			this.invoice_doc.posa_reservation_time = this.reservation_time;
			this.invoice_doc.posa_reservation_amount = this.reservation_amount;
			
			// Update due date based on credit_due_days
			if (this.credit_due_days) {
				const dueDate = new Date(this.reservation_time);
				dueDate.setDate(dueDate.getDate() + this.credit_due_days);
				this.invoice_doc.due_date = dueDate.toISOString().split('T')[0];
			}
			
			// Set payment amounts
			this.invoice_doc.payments.forEach((payment) => {
				if (payment.mode_of_payment.toLowerCase() === "cash") {
					payment.amount = this.reservation_amount;
				}
			});
			...
			this.eventBus.emit("show_message", {
				title: __("Reservation configured successfully"),
				color: "success",
			});
		},
		// Open reservation search dialog
		async open_reservation_search() {
			this.reservationSearchLoading = true;
			try {
				...
				this.eventBus.emit("show_message", {
					title: __("Unable to fetch reservations"),
					color: "error",
				});
			} finally {
				this.reservationSearchLoading = false;
			}
		},
		...
		async submit_invoice(print) {
			...
			try {
				...
				this.eventBus.emit("show_message", {
					title:
						this.invoice_doc.custom_is_reserve
							? __("Reservation {0} is Created", [r.message.name])
							: this.invoiceType === "Order" && this.pos_profile.posa_create_only_sales_order
				...
				if (!this.invoice_doc.custom_is_reserve) {
					...
					// If this is a reservation completion, release the reservation first
					if (this.invoice_doc.is_reservation_completion) {
						console.log("Completing reservation - releasing reserved stock:", submittedItems);
						submittedItems.forEach((item) => {
							const stockQty = item.stock_qty || item.qty || 0;
							const factor = item.conversion_factor || 1;
							const consumption = stockQty * factor;
							
							// Decrease reserved quantities
							const currentReserved = stockCoordinator.getReserved(item.item_code) || 0;
							const newReserved = Math.max(0, currentReserved - consumption);
							
							if (newReserved !== currentReserved) {
								stockCoordinator.updateReservations([{
									item_code: item.item_code,
									reserved_qty: newReserved
								}], { silent: true });
								console.log(`Released ${consumption} units of ${item.item_code} from reservation`);
							}
						});
					}
				} else {
					// For reservations, apply stock reservation instead of consumption
					console.log("Creating reservation - applying stock reservation for items:", submittedItems);
					stockCoordinator.applyInvoiceConsumption(submittedItems, {
						source: "reservation",
						reservation_id: this.invoice_doc.name,
					});
					console.log("Reservation created - stock reserved, not decreased");
				}
